    ...
    <section className="py-24 bg-gradient-to-br from-gray-50 via-white to-purple-50">
      <Container>
        <div className="text-center mb-16">
          <h2 className="text-5xl font-extrabold mb-4 max-w-4xl mx-auto leading-tight">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          <div className="bg-gradient-to-br from-gray-600 to-gray-700 text-white p-10 rounded-3xl shadow-2xl border border-gray-500">
            <h3 className="text-3xl font-bold mb-8 flex items-center gap-3">
              <span className="text-3xl opacity-50">❌</span>
              Самостоятельно
            </h3>
            {comparisons.map((item, index) => (
              <div key={index} className="mb-8 pb-8 border-b border-white/20 last:border-0">
                <div className="flex items-start gap-3 mb-3">
                  <span className="text-3xl opacity-70">✕</span>
                  <h4 className="text-2xl font-bold">{item.bad.title}</h4>
                </div>
                <p className="text-base opacity-90 leading-relaxed ml-11">{item.bad.description}</p>
              </div>
            ))}
          </div>

          <div className="bg-gradient-to-br from-[#EF4444] to-[#DC2626] text-white p-10 rounded-3xl shadow-2xl border border-red-400 relative overflow-hidden">
            ...
            {comparisons.map((item, index) => (
              <div key={index} className="mb-8 pb-8 border-b border-white/20 last:border-0 relative z-10">
                <div className="flex items-start gap-3 mb-3">
                  <span className="text-3xl">✓</span>
                  <h4 className="text-2xl font-bold">{item.good.title}</h4>
                </div>
                <p className="text-base opacity-95 leading-relaxed ml-11">{item.good.description}</p>
              </div>
            ))}
          </div>
        </div>
